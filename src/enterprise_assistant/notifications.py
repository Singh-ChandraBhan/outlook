...
def critical_incidents(records: list[KnowledgeRecord]) -> list[KnowledgeRecord]:
    ...
    for r in records:
        try:
            sla = float(
                r.metadata.get("sla_hours", r.metadata.get("sla_hours_remaining"))
            )
        except (TypeError, ValueError):
            sla = 999
        if r.record_type == "incident" and (
            r.priority.lower() in {"p1", "1", "critical"} or sla <= 4
        ):
            found[r.source_id] = r
...
def adaptive_card(records: list[KnowledgeRecord]) -> dict:
    facts = [
        {"title": r.source_id, "value": f"{r.priority or 'SLA'} — {r.title}"}
        for r in records
    ]
    return {
        "type": "message",
...
def notify_local(records: list[KnowledgeRecord], data_dir: str) -> str:
    folder = Path(data_dir) / "outbox"
    folder.mkdir(parents=True, exist_ok=True)
    path = (
        folder / f"alerts-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
    )
    payload = {
        "subject": "Enterprise critical incident alert",
...
def notify_teams(records: list[KnowledgeRecord], settings: Settings) -> None:
    ...
    try:
        response = requests.post(
            settings.teams_webhook_url, json=adaptive_card(records), timeout=30
        )
        response.raise_for_status()
    except requests.RequestException as exc:
        raise RuntimeError(
            f"Teams webhook request failed for {_safe_webhook(settings.teams_webhook_url)}"
        ) from exc
...
def notify_email(records: list[KnowledgeRecord], settings: Settings) -> None:
    ...
    payload = {
        "message": {
            ...
            "toRecipients": [
                {"emailAddress": {"address": settings.notification_email}}
            ],
        },
        "saveToSentItems": True,
    }
    response = requests.post(
        f"https://graph.microsoft.com/v1.0/users/{settings.graph_user_id}/sendMail",
